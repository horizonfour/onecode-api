...
ava_1.test.after('PUT /users/${id} - Atualizar usuario', async (t) => {
    const req = {
        method: 'PUT',
        url: `/users/${id}`,
        headers: {
...
ava_1.test.after('Remove mocked data from db', async (t) => {
    const item = await service.findOne({ name: MOCK_INSERT.name });
